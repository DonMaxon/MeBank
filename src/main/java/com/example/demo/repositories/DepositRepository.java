...
public interface DepositRepository extends CrudRepository<Deposit, UUID> {
    ...
    @Query(value = "SELECT info.name, info.maxSumm*(info.rate/100+1)*count(*) FROM Deposit GROUP BY info.name, info.maxSumm, info.rate ORDER BY info.maxSumm*(info.rate/100+1)*count(*) DESC")
